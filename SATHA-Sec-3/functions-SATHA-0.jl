...
#       "Self-Aware Transport of Heterogeneous Agents in Incomplete Markets" [SATHA] by Andrew Lyasoff
#
...
#=
gives equation (3.2); yyy is the value of *θᵤ(c); invh is the list of functions \hat{H} in the denominator
of (3.2)
nos = total number of states (7 in this example)
=#
function sumU(nos::Int64,yyy::Float64,sno::Int64,ccc::Float64,spot::Float64,invh,CPROB::Array{Float64,2},inc::Array{Float64,1},ι::Float64,R::Float64,beta::Float64)
    return sum([beta*dU(invh[k](inc[k]+yyy*ι)/ccc,R)*(ι)*CPROB[sno,k] for k=1:nos])-spot
end;

#=
for c ∈ grid, returns the solution *θᵤ(c) to equation (3.2);
the argument invh[k] is the inverse mapping in the denominator of (3.2)
(sno ~ u = current state, k ~ v = future state)
=#
function zroU(nos::Int64,sno::Int64,ccc::Float64,spot::Float64,invh,CPROB::Array{Float64,2},inc::Array{Float64,1},ι::Float64,R::Float64,beta::Float64)
    local g, zro, zroPrev, ZRO
    g=(yyy->sum([beta*dU(invh[k](inc[k]+yyy*ι)/ccc,R)*(ι)*CPROB[sno,k] for k=1:nos])-spot)
    if abs(g(0))<1e-14
        ZRO=0.0
    elseif g(0)>=1e-14
        zro=0.0;
...
## implements Step 2 in #3.2; returns the upper bound on the effective grid over consumption
function FindUpperC(nos::Int64,spot::Float64,invh,CPROB::Array{Float64,2},inc::Array{Float64,1},ι::Float64,R::Float64,beta::Float64)
    local tmp_bound::Float64, loc_inx::Float64, loc_iinx::Float64, loc_iiinx::Float64
    loc_inx=0.0;
    tmp_bound=loc_inx+0.0001;
    while tmp_bound>loc_inx && loc_inx<100100
        loc_inx+=10.0
        tmp_lst=[zroU(nos,k,Float64(loc_inx),spot,invh,CPROB,inc,ι,R,beta) for k=1:nos];
        tmp_bound=maximum([invh[k](inc[k]+tmp_lst[k]*ι) for k=1:nos]);
    end
    if loc_inx<=10000.0
        loc_iinx=loc_inx-10.0;
        tmp_bound=loc_iinx+0.0001;
        while tmp_bound>Float64(loc_iinx) && loc_iinx<loc_inx
            loc_iinx+=1.0
            tmp_lst=[zroU(nos,k,Float64(loc_iinx),spot,invh,CPROB,inc,ι,R,beta) for k=1:nos];
            tmp_bound=maximum([invh[k](inc[k]+tmp_lst[k]*ι) for k=1:nos]);
        end
        loc_iiinx=loc_iinx-1.0;
        tmp_bound=loc_iiinx+0.0001;
        while tmp_bound>Float64(loc_iiinx) && loc_iiinx<loc_iinx
            loc_iiinx+=1/10
            tmp_lst=[zroU(nos,k,Float64(loc_iiinx),spot,invh,CPROB,inc,ι,R,beta) for k=1:nos];
            tmp_bound=maximum([invh[k](inc[k]+tmp_lst[k]*ι) for k=1:nos]);
        end
        return loc_iiinx
    else
...
=#
function gtest(nos::Int64,state::Int64,pflo,spot::Float64,ccc::Float64,x::Array{Float64,1},CPROB::Array{Float64,2},inc::Array{Float64,1},ι::Float64,beta::Float64)
    return max(maximum([abs(x[i]+pflo[i](x[i])*spot-inc[i]-x[nos+1]*ι) for i=1:nos]), abs(sum([beta*dU(x[l]/ccc,R)*(ι)*CPROB[state,l] for l=1:nos])-spot))
end;
...
sol = list of future consumptions Τᵛ(u,c) followed by present portfolio *θᵤ(c) for c ∈ grid

sol[current_state][ cons_grid_location , list_of_fut_cons+current_portf ]
...
=#
function solve_sys(nos::Int64,g_step::Float64,gsz::Int64,spot::Float64,theta,invh,CPROB::Array{Float64,2},inc::Array{Float64,1},ι::Float64,R::Float64,beta::Float64)
    local gg_step::Float64, cgrid::StepRangeLen{Float64, Base.TwicePrecision{Float64}, Base.TwicePrecision{Float64}, Int64}, sol::Vector{Matrix{Float64}}, accu::Vector{Vector{Float64}}
    gg_step=g_step
    cgrid=make_grid(gg_step,gsz)
    sol=[zeros(gsz,nos+1) for k=1:nos]
    accu=[zeros(gsz) for k=1:nos]
        for k=1:nos # k ~ u = current state
            for i=1:gsz
                yloc=zroU(nos,k,Float64(cgrid[i]),spot,invh,CPROB,inc,ι,R,beta)
                accu[k][i]=sumU(nos,yloc,k,Float64(cgrid[i]),spot,invh,CPROB,inc,ι,R,beta)
                sol[k][i,:]=vcat([invh[kk](inc[kk]+yloc*ι) for kk=1:nos],yloc)   #* k ∼ u , kk ∼ v
            end
        end
    return sol, maximum(maximum([abs.(accu[k]) for k=1:nos])),maximum([maximum([gtest(nos,k0,theta,spot,cgrid[i],sol[k0][i,:],CPROB,inc,ι,beta) for i=1:gsz]) for k0=1:nos])
end;

#=
RETURNS: *θᵤ(·) as a spline over c ∈ grid for every u;
the future consumption mappings Τᵛ(u,·) as splines over a grid of length NCGSZ;
the inverses of Τᵛ(u,·) as splines
=#
function compute_next(nos::Int64,g_step::Float64,gsz::Int64,sol::Array{Array{Float64,2},1},NCGSZ::Int64)
    local gg_step::Float64, cgrid::StepRangeLen{Float64, Base.TwicePrecision{Float64}, Base.TwicePrecision{Float64}, Int64}, θnew, cnext, invcnext, gcmid::Float64
    gg_step=g_step
    cgrid=make_grid(gg_step,gsz)
...
function generate_dist_0(nos::Int64,upper_b::Float64,invCfun,CPROB::Array{Float64,2},BSP::Array{Float64,1},FGSZ::Int64)
    local F,GF::StepRangeLen{Float64, Base.TwicePrecision{Float64}, Base.TwicePrecision{Float64}, Int64},Fval::Matrix{Float64}
...
#=
iterates (3.3) starting with supplied distribution (performs Step 5 in #3.2)
returns the distribution calculated in Step 5 as a list of spline;
...
#=
repeats Steps 1-6 in 3.2 until the market clearing #3.2-(b) holds
INPUTS: total number of idiosyncratic states;
step-size of the grid over c;
number of grid points;
previous portfolios †θ;
inverse mappings in the denominator of (3.2) constructed from †θ;
the most recent approximation of the spot price of the bond;
the transition probability matrix;
the vector of paychecks for the employment groups;
the list of steady-state probabilities;
(constant) aggregate income in the economy;
...
the list of portfolios *θᵤ(·) as splines;
the future consumptions Tᵛ(u,·) as splines;
the inverses of Tᵛ(u,·) as splines; 
...
=#
function main_prog(nos::Int64,g_step::Float64,gsz::Int64,theta,invh,spot::Float64,CPROB::Array{Float64,2},inc::Array{Float64,1},BSP::Array{Float64,1},ι::Float64,R::Float64,fval::Array{Float64,2},HRGSZ::Int64,NCGSZ::Int64,FGSZ::Int64,beta::Float64)
    local gg_step::Float64, cgrid::StepRangeLen{Float64, Base.TwicePrecision{Float64}, Base.TwicePrecision{Float64}, Int64}, clearing::Float64, θ, invH, SPOT::Float64, F, upper_bb::Float64, Fval::Matrix{Float64}, θnew, cnext, invcnext, sol, accu0::Float64, accu::Float64, ANSATZ, CLR, c_high::Float64, gcmid::Float64
##
    Fval=fval
    SPOT=spot
    invH=invh
    ...
    while clr_check==0&&no_iiter<25
        no_iiter+=1;
        sol,accu0,accu=solve_sys(nos,gg_step,gsz,SPOT,θ,invH,CPROB,inc,ι,R,beta)
        ...
        elseif length(ANSATZ)>=2
            # generate the next guess for the spot
            # the zero of the line drawn from the last two spots and the last two clearing values
            SPOT=(ANSATZ[end]*CLR[end-1]-ANSATZ[end-1]*CLR[end])/(CLR[end-1]-CLR[end])
        end;
        #
        # because SPOT changes, the right side of (3.1) changes
        # invH has to be recomputed -- with the old theta and the new spot
        #
...
(generates those internally as an initial guess)
=#
function main_prog_0(nos::Int64,g_step::Float64,gsz::Int64,theta,invh,spot::Float64,CPROB::Array{Float64,2},inc::Array{Float64,1},BSP::Array{Float64,1},ι::Float64,R::Float64,HRGSZ::Int64,NCGSZ::Int64,FGSZ::Int64,beta::Float64)
    local gg_step::Float64, cgrid::StepRangeLen{Float64, Base.TwicePrecision{Float64}, Base.TwicePrecision{Float64}, Int64}, clearing::Float64, θ, invH, SPOT, F, upper_bb::Float64, Fval::Matrix{Float64}, θnew, cnext, invcnext, sol, accu0::Float64, accu::Float64, ANSATZ, CLR, c_high::Float64, gcmid::Float64
    #
    SPOT=spot
    invH=invh
    θ=theta
...
#=
runs Steps 1-7 in #3.2 for a specified number of iterations
...
the total number of grid points in the *ranges* of the future consumption mappings Τᵛ(u,·) (used for inversion);
the total number of abscissas in the grid used to interpolate the distribution of households;
the impatience parameter β
...
the future consumptions Τᵛ(u,·) as splines; the inverses of Τᵛ(u,·) as splines;
...
=#
function find_equil(nos::Int64,start_iter::Int64,nmb_of_iter::Int64,conv_tol::Float64,g_step::Float64,gsz::Int64,theta,invh,spot::Float64,CPROB::Array{Float64,2},inc::Array{Float64,1},BSP::Array{Float64,1},ι::Float64,R::Float64,HRGSZ::Int64,NCGSZ::Int64,FGSZ::Int64,beta::Float64)
    local gg_step::Float64, cgrid::StepRangeLen{Float64, Base.TwicePrecision{Float64}, Base.TwicePrecision{Float64}, Int64}, no_iter::Int64, solPrev, invH, SPOT::Float64, θ, SPOTprev::Float64, accuPrev::Float64, accu0Prev::Float64, θprev, conv_check::Float64, conv_check_c::Float64, clearing::Float64, F, upper_bb::Float64, Fval::Matrix{Float64}, θnew, cnext, invcnext, sol, accu0::Float64, accu::Float64, ANSATZ, CLR, c_high::Float64, gcmid::Float64
#
    SPOT=spot
    invH=invh
    θ=theta
    ...
    no_iter+=1;
    println(no_iter)
    clearing, F, upper_bb, Fval, θnew, cnext, invcnext, gg_step, sol, accu0, accu, ANSATZ, CLR = main_prog_0(nos,gg_step,gsz,θ,invH,SPOT,CPROB,inc,BSP,ι,R,HRGSZ,NCGSZ,FGSZ,beta);
    SPOTprev=SPOT;
    SPOT=ANSATZ[end];
    accuPrev=accu;
    accu0Prev=accu0;
    ...
    while no_iter<nmb_of_iter&&conv_check>conv_tol
        no_iter+=1;
        println(no_iter)
        clearing, F, upper_bb, Fval, θnew, cnext, invcnext, gg_step, sol, accu0, accu, ANSATZ, CLR = main_prog(nos,gg_step,gsz,θ,invH,SPOT,CPROB,inc,BSP,ι,R,Fval,HRGSZ,NCGSZ,FGSZ,beta);
        cgrid=make_grid(gg_step,gsz)
        gcmid=(cgrid[end]+cgrid[1])/2.0
