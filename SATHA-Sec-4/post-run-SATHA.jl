...
#                
# Supplements the paper "Self-Aware Transport of Heterogeneous Agents in Incomplete Markets" [SATHA]
...
# not needed run-SATHA.jl is executed
@time begin
    ...
    include("ini-setup-SATHA.jl"); # include("ini-setup-SATHA-mod.jl") to change β from original
...
out2=deserialize("output-SATHA.jls");
...
## provides the left plot in Figure 10 in SATHA
begin
...
## provides the right plot in Figure 10 in SATHA
begin
    #plot_grid=collect(grid_ave[1]:0.005:grid_ave[end])
    plot_grid=collect(0.5:0.005:1.0)
...
## provides the plot in Figure 11 in SATHA
begin
    #plot_grid=collect(grid_ave[1]:0.005:grid_ave[end])
    plot_grid=collect(0.5:0.005:1.0)
...
# (0.003536019154666614, 0.0035297086608960804)

#uniform distance between solid lines
maximum(abs.((T11).-T12))
# 0.006634469204318982
