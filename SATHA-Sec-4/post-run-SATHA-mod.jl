...
#                "Self-Aware Transport of Heterogeneous Agents in Incomplete Markets" [SATHA]
...
# not needed if run-SATHA-mod.jl is executed
@time begin
    ...
    include("functions-SATHA.jl");
    include("ini-setup-SATHA-mod.jl"); # include("ini-setup-SATHA-mod.jl") to change β from original
...
# not needed if run-SATHA.jl is executed
out2=deserialize("output-SATHA-mod.jls");
...
## provides the left plot in Figure 10 (not in SATHA)
begin
...
## provides the right plot in Figure 10 (not in SATHA)
begin
    #plot_grid=collect(grid_ave[1]:0.005:grid_ave[end])
    plot_grid=collect(0.5:0.005:1.0)
...
## provides the plot in Figure 11 (not in SATHA)
begin
    #plot_grid=collect(grid_ave[1]:0.005:grid_ave[end])
    plot_grid=collect(0.5:0.005:1.0)
...
## provides the right plot in Figure 12 (not in SATHA)
begin
    #plot_grid=collect(grid_ave[1]:0.005:grid_ave[end])
    plot_grid=collect(0.5:0.005:1.0)
...
# (0.005499037773440518, 0.00547814043916095)
...
## provides the left plot in Figure 13 (not in SATHA)
begin
    #plot_grid=collect(grid_ave[1]:0.005:grid_ave[end])
    plot_grid=collect(0.5:0.005:1.0);
...
# 8.625025034181277e-5


## transport in terms of capital into low state
## provides the right plot in Figure 13 (not in SATHA)
begin
    #plot_grid=collect(grid_ave[1]:0.005:grid_ave[end])
    plot_grid=collect(0.5:0.005:1.0);
