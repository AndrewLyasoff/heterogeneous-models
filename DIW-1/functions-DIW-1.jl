...
# system of forst order Lagrange condition (FOLC) to solve equations to solve -- see (4.13) and (4.12)
...
# the left side of the market clearing condition -- see (4.6) and (4.10)
...
#=
solves the system comprised of (4.12) and (4.13) for the 10 unknown intercepts (8 for the future consumption lines
...
or steps (2) through (5) in the initial backward step  from (4.16)
=# 
...
at a given (fixed) point on the ℝ²-grid performs steps (1) through (8) in the generic backward step from (4.16)
=#                
...
#=
completes the initial backward step from (4.16) in DIW for all points on the ℝ²-grid with parallelization:
...
#=
repeats the generic backward step from (4.16) in DIW for all points on the ℝ²-grid with parallelization,
