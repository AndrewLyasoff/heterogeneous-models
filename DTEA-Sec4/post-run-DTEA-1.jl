...
#                
# Supplements the paper "Dynamic transportation of economic agents" [DTEA]
...
@time begin
    ...
    include("functions-DTEA-1.jl");
    include("ini-setup-DTEA-1.jl"); # include("ini-setup-DTEA-1-mod.jl") to change β from original
...
# not needed if run-DTEA-1.jl
out2=deserialize("output-DTEA-1.jls");
...
# plot the difference
# provides the left plot in ([DTEA], Figure 15)
begin
...
#plot the difference
# provides the right plot in ([DTEA], Figure 15)
begin
...
## provides the left plot on ([DTEA], Figure 10)
begin
...
# provides the right plot in ([DTEA], Figure 10)
begin
...
# provides the left plot in ([DTEA], Figure 11)
begin
...
#provides the right plot in ([DTEA], Figure 11)
begin
...
#=
2-element Vector{Float64}:
 1.4316317189067007e-5
 1.8557635771077052e-5
=#


# distribution transport from 1 to 1
# provides the left plot on ([DTEA], Figure 12)
begin
...
# provides the right plot on ([DTEA], Figure 12)
begin
...
# provides the left plot on ([DTEA], Figure 13)
begin
...
# provides the right plot on ([DTEA], Figure 13)
begin
...
#1.412337 seconds (16.54 M allocations: 1.478 GiB, 23.31% gc time, 1.00% compilation time)
...
# provides the left plot on ([DTEA], Figure 14)
begin
...
# # provides the right plot on ([DTEA], Figure 14)
begin
...
## RECRE THE SIMULATION RESULTS FROM KRUSELL AND SMITH
...
#  4.922140 seconds (16.41 M allocations: 1.059 GiB, 13.27% gc time, 98.51% compilation time)
#  0.091247 seconds (412 allocations: 145.100 MiB, 24.88% gc time)
...
#=
2-element Vector{Float64}:
 1.3530374957211505e-6
 5.510604179821965e-7
=#
...
deviance(ols1)
# 4.5884379354287326e-5

sum((residuals(ols1)).^2) # same as deviance(ols1)
# 4.5884379354287454e-5 
...
#=
2-element Vector{Float64}:
 1.2686614290316024e-6
 5.197696409694212e-7
=#
