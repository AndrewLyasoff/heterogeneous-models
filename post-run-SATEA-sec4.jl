...
#                
# Supplements the paper "The Time-Interlaced Self-Consistent Master System of Heterogeneous-Agent Models" [SATEA] by Andrew Lyasoff
#
...
# not needed run-SATEA-sec4.jl is executed
@time begin
    ...
    include("ini-setup-SATEA-sec4.jl"); # include("ini-setup-SATEA-sec4-mod.jl") to change β from original
...
# not needed if run-SATEA-sec4.jl is executed
out2=deserialize("output-SATEA-sec4.jls");
...
## provides the left plot in Figure 10 in SATEA
begin
...
## provides the right plot in Figure 10 in SATEA
begin
    #plot_grid=collect(grid_ave[1]:0.005:grid_ave[end])
    plot_grid=collect(0.5:0.005:1.0)
...
## provides the plot in Figure 11 in SATEA
begin
    #plot_grid=collect(grid_ave[1]:0.005:grid_ave[end])
    plot_grid=collect(0.5:0.005:1.0)
